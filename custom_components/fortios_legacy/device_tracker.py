...
from homeassistant.const import CONF_HOST, CONF_TOKEN, CONF_VERIFY_SSL, CONF_DELAY_TIME, CONF_WHITELIST
...
CONF_MAX_LAST_SEEN = "max_last_seen"
...
PLATFORM_SCHEMA = BASE_PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_MAX_LAST_SEEN, default=60): cv.positive_int,
        vol.Optional(CONF_WHITELIST, default=DEFAULT_LIST): cv.string,
        vol.Optional(CONF_BLACKLIST, default=DEFAULT_LIST): cv.string
    }
)
...
class FortiOSDeviceScanner(DeviceScanner):
    ...
    def __init__(self, fgt, config) -> None:
        """Initialize the scanner."""
        self._clients: list[str] = []
        self._clients_json: dict[str, Any] = {}
        self._fgt = fgt
        self._max_last_seen = config[DOMAIN][CONF_MAX_LAST_SEEN]
        self._whitelist = config[DOMAIN][CONF_WHITELIST]
        self._blacklist = config[DOMAIN][CONF_BLACKLIST]
        ...
        if clients_json:
            try:
                for client in clients_json["results"]:
                    self._add_client = None

                    if ( "tagging" in client ):
                        for tags in client["tagging"]:
                            for tag in tags["tags"]:
                                if ( tag == self._blacklist or self._add_client is False ):
                                    self._add_client = False
                                    continue

                                if (
                                    tag == self._whitelist
                                    and int(client["last_seen"]) < self._max_last_seen
                                    and "master_mac" in client
                                ):
                                    self._add_client = True

                    if ( self._whitelist == "NULL" and self._add_client is None ):
                        if (
                            int(client["last_seen"]) < self._max_last_seen
                            and "master_mac" in client
                        ):
                            self._add_client = True
    ...
    def get_device_name(self, device):
        ...
        for client in data["results"]:
            if "master_mac" in client and client["master_mac"] == device:
                try:
                    name = client["host"]["name"].replace(" ", "_")
                except:
